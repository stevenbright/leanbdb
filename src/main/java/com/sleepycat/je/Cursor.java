...
public class Cursor implements ForwardCursor {
    ...
    public OperationResult put(
        ...
        try {
            ...
            return putInternal(
                key, data, putType, options, putType.getCursorStat());
    ...
    OperationResult putInternal(
        ...
        if (thrput != null && stat >= 0) {
            thrput.increment(stat);
        }
    ...
    OperationResult getInternal(
        ...
        if (searchMode != null) {
            ...
            return search(key, data, lockMode, cacheMode, searchMode);
        }
     ...
     * triggers and prevents phantoms. Note that although putNotify is called,
     * secondaries are not populated because this cursor is internal and has no
    ...
    private OperationResult putNotify(
        ...
        try {
            ...
            if (putMode == PutMode.CURRENT) {
                ...
                final Pair<OperationResult, Boolean> ret = putNoNotify(
                    key, data, ln, cacheMode, expInfo, putMode,
     ...
     * @return pair of result and 'inserted' boolean.
     */
    private Pair<OperationResult, Boolean> putNoNotify(
        final DatabaseEntry key,
        final DatabaseEntry data,
        final LN ln,
        final CacheMode cacheMode,
        ...
        try {
            ...
            /* Perform operation. */
            Pair<OperationResult, Boolean> ret = dup.insertOrUpdateRecord(
    ...
    private OperationResult putCurrentNoNotify(
        ...
        final OperationResult result = cursorImpl.updateCurrentRecord(
            key, data, expInfo, returnOldData, returnNewData,
            repContext, thrput);
    ...
    OperationResult position(
        ...
        synchronized (getTxnSynchronizer()) {
            ...
            if (dbImpl.getSortedDuplicates()) {
                return positionHandleDups(
                    key, data, lockMode, cacheMode, first);
            }

            return positionNoDups(key, data, lockMode, cacheMode, first);
        }
    }
    ...
    OperationResult retrieveNext(
        ...
        if (dbImpl.getSortedDuplicates()) {
            return retrieveNextHandleDups(
                key, data, lockMode, cacheMode, getMode);
        }

        return retrieveNextNoDups(key, data, lockMode, cacheMode, getMode);
    }
    ...
    OperationResult search(
        ...
        synchronized (getTxnSynchronizer()) {
            ...
            if (dbImpl.getSortedDuplicates()) {
                ...
                switch (searchMode) {
                case SET:
                    return dupsGetSearchKey(key, data, lockMode, cacheMode);
                case SET_RANGE:
                    return dupsGetSearchKeyRange(
                        key, data, lockMode, cacheMode);
                case BOTH:
                    return dupsGetSearchBoth(key, data, lockMode, cacheMode);
                case BOTH_RANGE:
                    return dupsGetSearchBothRange(
                        key, data, lockMode, cacheMode);
                default:
                    throw EnvironmentFailureException.unexpectedState(
                        searchMode.toString());
     ...
     * been locked.
     *
     * @return status plus primary record version.  The status is true if
     * the primary was read successfully, or false in one of the following
     * cases:
     ...
     */
    Pair<Boolean, RecordVersion> readPrimaryAfterGet(
        ...
        try {
            ...
            return new Pair<>(true, priCursor.getCachedRecordVersion());
        } finally {
            priCursor.close();
        }
    }
