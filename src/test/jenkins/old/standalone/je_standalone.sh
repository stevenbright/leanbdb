...
ERROREXTRACTSCRIPT="error_extract_je_standalone.sh"
...
while getopts "O:j:t:b:l:T:" OPTION
do
	case $OPTION in
		O)
			TEST_ARG=$OPTARG
...
cd ~
mkdir -p je_standalone_test_result_archive
cd je_standalone_test_result_archive
if [ -d ${TASK_NAME} ]; then
    rm -rf ${TASK_NAME}
fi
mkdir -p ${TASK_NAME}
...
mkdir -p ${TEMPDIRNAME}
SAVEPATH=~/je_standalone_test_result_archive/${TASK_NAME}/${TEMPDIRNAME}

if [ -d ${BASEDIR}/${TASK_NAME} ] ; then
	cd ${BASEDIR}/${TASK_NAME} && hg clone -b default ssh://${JEREPUSER}@sleepycat-scm.us.oracle.com://a/hgroot/je
else
	mkdir -p ${BASEDIR}/${TASK_NAME}
	cd ${BASEDIR}/${TASK_NAME} && hg clone -b default ssh://${JEREPUSER}@sleepycat-scm.us.oracle.com://a/hgroot/je
fi

if [ -f ${BASEDIR}/${TASK_NAME}/je/${CHANGESETFILE} ] ; then
        rm ${BASEDIR}/${TASK_NAME}/je/${CHANGESETFILE}
fi

cd je && hg log -l 1 -v >> ./${CHANGESETFILE} && cd ..
...
BUILD_VER=`cd $ROOT_DIR/je && hg parent`
...
do_standalone_test() {
        if [ "${TASK_NAME}" == "je_standalone_repdbops" ]; then
            pkill -9 sar
            sar -A 1 100000 > ~/standalone.sar.output &
        fi

	date_start=`date +"%s"`
	cd ${ROOT_DIR}/je && ant -Dtestcase="$1" -Dargs="$7" standalone
	retvalue=$?
	if [ "${retvalue}" != 0 ]; then
	...
	cd ~/ && bash ${ERROREXTRACTSCRIPT} ${ROOT_DIR}/${JESTANDALONEPATH}/log output.log "JE.Standalone" "$2" $intervel ${ROOT_DIR}/je/"$3" "${retvalue}" "$5" "$6"
	cd ${TEST_DIR} && cp log "$4"
        cp -r ${TEST_DIR} ${SAVEPATH}/standalone_$2
        if [ "${TASK_NAME}" == "je_standalone_repdbops" ]; then
            pkill -9 sar
            mv ~/standalone.sar.output ${SAVEPATH}/standalone_$2/
        fi
}

# ant -Dtestcase=*** standalone
do_standalone_test "${TESTNAME}" "${TESTNAME}" "${XMLRESULT1}" "${LOGRESULT1}" "${ERRORPATTERN}" "${IGNORECASE}" "${TESTARG1}"

# ant -Dtestcase=*** -Dargs=*** standalone
if [ X"${TESTNAMEWITHARG2}" != X"" ]; then
	do_standalone_test "${TESTNAME}" "${TESTNAMEWITHARG2}" "${XMLRESULT2}" "${LOGRESULT2}" "${ERRORPATTERN}" "${IGNORECASE}" "${TESTARG2}"
fi

# ant -Dtestcase=*** -Dargs=*** standalone
if [ X"${TESTNAMEWITHARG3}" != X"" ]; then
	do_standalone_test "${TESTNAME}" "${TESTNAMEWITHARG3}" "${XMLRESULT3}" "${LOGRESULT3}" "${ERRORPATTERN}" "${IGNORECASE}" "${TESTARG3}"
fi
