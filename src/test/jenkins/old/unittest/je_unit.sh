...
while getopts "O:j:t:b:l:h:p:" OPTION
do
	case $OPTION in
		O)
			TEST_ARG1=$OPTARG
...
#    else
#        echo "Host: slc04arq.us.oracle.com" >> ${ROOT_DIR}/je/location_of_environment_and_log.txt
#    fi
#fi
BUILDID=`ssh -l jenkins slc04ark "cat /scratch/jenkins/jobs/${TASK_NAME}/nextBuildNumber"`
BUILDID=`expr $BUILDID - 1`
...
if [ "$TEST_ARG1" = "-Dalltests=true" ] ; then
    if [ X$JDK_VERSION == X"7" ] ; then
        echo "Host: slc00brj.us.oracle.com" >> ${ROOT_DIR}/je/location_of_environment_and_log.txt
    else
        echo "Host: slc04arm.us.oracle.com" >> ${ROOT_DIR}/je/location_of_environment_and_log.txt
    fi
else
