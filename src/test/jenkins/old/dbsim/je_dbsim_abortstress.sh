...
cp ~/recovery.conf ~/${TASK_NAME}/dbsim/configs/
cp ~/abortstress.conf ~/${TASK_NAME}/dbsim/configs/
...
save_results() {
	echo "Get the files"
        ...
        BUILDID=`ssh -l jenkins slc04ark "cat /scratch/jenkins/jobs/je_dbsim_abortstress/nextBuildNumber"`
        BUILDID=`expr $BUILDID - 1`
        LOGLOCATION=~/log_archive/je_dbsim_abortstress/$BUILDID
        mkdir -p $LOGLOCATION
	...
	cd ${JE_DIR} && scp jenkins_changeset.txt location_of_environment_and_log.txt jenkins@slc04ark:~/jobs/${TASK_NAME}/workspace/
        ...
        scp ${DBSIM_DIR}/\[log\]DBSim_AbortStress_* jenkins@slc04ark:~/jobs/${TASK_NAME}/workspace/
...
dbsim() {
	...
	do
		...
		#run_dbsim
                ...
                cd ~/ && bash error_substract_dbsim.sh ${DBSIM_DIR}/Case_${CASE}-JDK_${JAVA_VERN}.tmp output.log "JE.DBSim" "AbortStress" $intervel ${ROOT_DIR}/test_${JAVA_VERN}.xml
                tar czf ${DBSIM_DIR}/[tar]DBSim_AbortStress_${VER_STR}.tar.gz ${DBSIM_DIR}/Case_${CASE}-JDK_${JAVA_VERN}.tmp
                tail ${DBSIM_DIR}/Case_${CASE}-JDK_${JAVA_VERN}.tmp -n 100 > ${DBSIM_DIR}/[log]DBSim_AbortStress_${VER_STR}.log
