...
case ${TASK_NAME} in
	je_dbsim_abortstress)
		CASE="abortstress"
		cp ~/${CASE}.conf ${ROOT_DIR}/dbsim/configs/
		;;
	je_dbsim_duplicate)
		CASE="duplicate"
		VERIFYITER=100
...
cd ${BASEDIR} && bash ${ERROREXTRACTSCRIPT} ${DBSIM_DIR}/Case_${CASE}.tmp output.log "JE.DBSim" "${CASE}" $intervel ${ROOT_DIR}/test.xml "0" "${ERRORPATTERN}" "${IGNORECASE}" "${EXCEPTION_EXPRS}"
