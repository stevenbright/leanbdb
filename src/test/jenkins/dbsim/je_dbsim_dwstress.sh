...
save_results() {
	echo "Get the files"
        ...
        BUILDID=`ssh -l jenkins slc04ark "cat /scratch/jenkins/jobs/je_dbsim_dwstress/nextBuildNumber"`
        BUILDID=`expr $BUILDID - 1`
        LOGLOCATION=~/log_archive/je_dbsim_dwstress/$BUILDID
        mkdir -p $LOGLOCATION
	...
	scp ${DBSIM_DIR}/\[log\]DBSim_DWStress_* jenkins@slc04ark:~/jobs/${TASK_NAME}/workspace/
}
...
dbsim() {
	...
	do
		...
		cd ~/ && bash error_substract_dbsim.sh ${DBSIM_DIR}/Case_${CASE}-JDK_${JAVA_VERN}.tmp output.log "JE.DBSim" "DWStress" $intervel ${ROOT_DIR}/test_${JAVA_VERN}.xml
		tar czf ${DBSIM_DIR}/[tar]DBSim_DWStress_${VER_STR}.tar.gz ${DBSIM_DIR}/Case_${CASE}-JDK_${JAVA_VERN}.tmp
		tail ${DBSIM_DIR}/Case_${CASE}-JDK_${JAVA_VERN}.tmp -n 100 > ${DBSIM_DIR}/[log]DBSim_DWStress_${VER_STR}.log
                #if [ $? -eq 0 ] ; then
