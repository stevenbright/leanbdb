...
run_dbsim() {
	...
	case ${CASE} in 
		"recovery"|"duplicate")
			java -cp $CLASSPATH -DsetErrorListener=true -ea ${run_class} -h ${TEST_DIR} -c ${DBSIM_DIR}/configs/${CASE}.conf -V 100 -B >& ${DBSIM_DIR}/Case_${CASE}-JDK_${JAVA_VERN}.tmp
			;;
		"abortstress"|"dwStress")
...
save_results() {
	echo "Get the files"
        BUILDID=`ssh -l jenkins slc04ark "cat /scratch/jenkins/jobs/je_dbsim_duplicate/nextBuildNumber"`
        BUILDID=`expr $BUILDID - 1`
        LOGLOCATION=~/log_archive/je_dbsim_duplicate/$BUILDID
        mkdir -p $LOGLOCATION

        cd $LOGLOCATION
        echo "Host: slc04arn.us.oracle.com" >> ${JE_DIR}/location_of_environment_and_log.txt
	...
	cd ${JE_DIR} && scp jenkins_changeset.txt location_of_environment_and_log.txt jenkins@slc04ark:~/jobs/${TASK_NAME}/workspace/
        ...
        scp ${DBSIM_DIR}/\[log\]DBSim_Duplicate_* jenkins@slc04ark:~/jobs/${TASK_NAME}/workspace/
...
dbsim() {
	...
	do
		...
		#run_dbsim
                ...
                #grep -i "Exception" ${DBSIM_DIR}/Case_${CASE}-JDK_${JAVA_VERN}.tmp

		date_end=`date +"%s"`
                intervel=$[$date_end - $date_start]
                cd ~/ && bash error_substract_dbsim.sh ${DBSIM_DIR}/Case_${CASE}-JDK_${JAVA_VERN}.tmp output.log "JE.DBSim" "Duplicate" $intervel ${ROOT_DIR}/test_${JAVA_VERN}.xml
                tar czf ${DBSIM_DIR}/[tar]DBSim_Duplicate_${VER_STR}.tar.gz ${DBSIM_DIR}/Case_${CASE}-JDK_${JAVA_VERN}.tmp
                tail ${DBSIM_DIR}/Case_${CASE}-JDK_${JAVA_VERN}.tmp -n 100 > ${DBSIM_DIR}/[log]DBSim_Duplicate_${VER_STR}.log


                #if [ $? -eq 0 ] ; then
                #        echo "${CASE} on ${JAVA_VERN} FAILED"
                #fi
