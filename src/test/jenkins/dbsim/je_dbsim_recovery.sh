...
save_results() {
	echo "Get the files"
        ...
        BUILDID=`ssh -l jenkins slc04ark "cat /scratch/jenkins/jobs/je_dbsim_recovery/nextBuildNumber"`
        BUILDID=`expr $BUILDID - 1`
        LOGLOCATION=~/log_archive/je_dbsim_recovery/$BUILDID
        mkdir -p $LOGLOCATION
	...
	scp ${ROOT_DIR}/test_* jenkins@slc04ark:~/jobs/${TASK_NAME}/workspace/
        scp ${DBSIM_DIR}/\[log\]DBSim_Recovery_* jenkins@slc04ark:~/jobs/${TASK_NAME}/workspace/
...
dbsim() {
	...
	do
		...
		date_end=`date +"%s"`
                ...
                cd ~/ && bash error_substract_dbsim.sh ${DBSIM_DIR}/Case_${CASE}-JDK_${JAVA_VERN}.tmp output.log "JE.DBSim" "Recovery" $intervel ${ROOT_DIR}/test_${JAVA_VERN}.xml
                tar czf ${DBSIM_DIR}/[tar]DBSim_Recovery_${VER_STR}.tar.gz ${DBSIM_DIR}/Case_${CASE}-JDK_${JAVA_VERN}.tmp
                tail ${DBSIM_DIR}/Case_${CASE}-JDK_${JAVA_VERN}.tmp -n 100 > ${DBSIM_DIR}/[log]DBSim_Recovery_${VER_STR}.log
