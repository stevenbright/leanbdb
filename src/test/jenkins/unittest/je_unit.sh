...
# hg clone je
if [ "${TASK_NAME}" == "je_unit_aix" ]; then
	ssh tests@slc04arq "rm -rf ~/${TASK_NAME} && mkdir -p ~/${TASK_NAME}"
        echo "hg clone -b ${BRANCH} ${JEREVISIONARG} ssh://${JEREPUSER}@${JEREPO}"
	ssh tests@slc04arq "cd ~/${TASK_NAME} && ${HGPATH}hg clone -b ${BRANCH} ${JEREVISIONARG} ssh://${JEREPUSER}@${JEREPO} ./je"
	ssh tests@slc04arq "cd ~/${TASK_NAME}/je && hg log -l 1 -v > ./jenkins_changeset.txt"
        BUILD_VER=`ssh tests@slc04arq "cd ~/${TASK_NAME}/je && hg parent"`
	rm -rf ${BASEDIR}/${TASK_NAME}
	scp -r tests@slc04arq:~/${TASK_NAME} ${BASEDIR}
else
...
if [ X$JDK_VERSION == X"8" ] ; then
	...
	export JAVA_HOME=${BASEDIR}/app/ibm-java-ppc64-71
else
