...
while getopts "O:j:t:R:b:r:l:T:" OPTION
do
	case $OPTION in
		O)
			TEST_ARG=$OPTARG
...
cd ${BASEDIR}/${TASK_NAME} && hg clone -b ${BRANCH} ${JEREVISIONARG} ssh://${JEREPUSER}@${JEREPO} ./je
...
scp ${JENKINSVM}:/scratch/jenkins/bin/error_extract_je.sh ${BASEDIR}/
scp ${JENKINSVM}:/scratch/jenkins/bin/gen_xml.sh  ${BASEDIR}/
...
do_standalone_test() {
	...
	cd ${BASEDIR} && bash ${ERROREXTRACTSCRIPT} ${ROOT_DIR}/${JESTANDALONEPATH}/log output.log "JE.Standalone" "$2" $intervel ${ROOT_DIR}/je/"$3" "${retvalue}" "$5" "$6" "$7"
